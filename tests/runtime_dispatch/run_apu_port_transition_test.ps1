...
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $out)) {
    throw "gcc did not produce the APU transition test executable"
}
& $out
if ($LASTEXITCODE -ne 0) { throw "APU transition test failed" }
